// ══════════════════════════════════════════
// mantenimiento.js  —  Magnoliga (API version)
// ══════════════════════════════════════════
...
async function renderMedia() {
  ...
  let items = [];
  try {
    items = await api('/api/arquitectura/');
  } catch(e) { return; }
  ...
  grid.querySelectorAll('.remove-btn').forEach(btn => {
    btn.addEventListener('click', async function() {
      ...
      try {
        await api(`/api/arquitectura/${this.dataset.id}/`, 'DELETE');
        await renderMedia();
        showToast('Eliminado');
      } catch(e) {
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  document.getElementById('submitMedia').addEventListener('click', async () => {
    ...
    try {
      await api('/api/arquitectura/', 'POST', fd);
      modal.hide();
      await renderMedia();
